...
local get_compiled_path = function(theme)
    return table.concat({vim.fn.stdpath("state"), "yorumi", theme .. "_compiled.lua"}, PATH_SEP)
end

---@return string theme
function M.get_theme_from_bg_opt()
    local config = require("yorumi").config
...
function M.compile(theme, highlights, termcolors)
    vim.loop.fs_mkdir(vim.fn.stdpath("state") .. PATH_SEP .. "yorumi", 448)

    local fname = get_compiled_path(theme)
    local file, err = io.open(fname, "wb")
    if not file or err then
        vim.notify("yorumi: Error writing " .. fname .. ":\n" .. err, vim.log.levels.ERROR)
        return
    end

    local lines = {
        "require'yorumi'.compiled = string.dump(function()",
        "local g = vim.g",
    ...
    file:write(require("yorumi").compiled)
    file:close()
end
